...
export interface KnowledgeItem {
  ...
  source: SourceRef; tags: string[]; importance: number; updatedAt: string;
}
...
export interface FileRecord {
  path: string; language: Language; size: number; hash: string;
  imports: string[]; symbols: SymbolRecord[]; tests: boolean;
}
export interface RepositoryIndex {
  repositoryId: string; revision: string; files: FileRecord[]; symbols: SymbolRecord[];
  dependencies: DependencyEdge[]; detectedLanguages: Array<{ language: Language; files: number }>;
  generatedAt: string;
}
export interface ProjectBrain {
  projectId: string; revision: string; summary: string;
  requirements: KnowledgeItem[]; decisions: KnowledgeItem[]; lessons: KnowledgeItem[];
  risks: KnowledgeItem[]; conventions: KnowledgeItem[]; index: RepositoryIndex;
  updatedAt: string;
}
...
const EXTENSIONS: Record<string, Language> = {
  ...
  '.py':'python','.java':'java','.go':'go','.rs':'rust','.c':'c','.h':'c','.cc':'cpp','.cpp':'cpp',
  '.hpp':'cpp','.cs':'csharp','.php':'php','.rb':'ruby','.kt':'kotlin','.kts':'kotlin',
  '.swift':'swift','.dart':'dart','.sql':'sql','.sh':'shell','.bash':'shell','.zsh':'shell',
  '.html':'html','.css':'css'
};
...
function stableHash(input: string): string {
  ...
  for (let i=0;i<input.length;i++) { h1 ^= input.charCodeAt(i); h1 = Math.imul(h1, h2); }
...
function extractSymbols(path: string, text: string, language: Language): SymbolRecord[] {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(text))) {
      const name = match[1];
      if (!name) continue;
      const kind = pattern.source.includes('class') ? 'class' : pattern.source.includes('interface') ? 'interface' : pattern.source.includes('type') ? 'type' : 'function';
...
function extractImports(text: string, language: Language): string[] {
  const patterns: RegExp[] = language === 'python' ? [/^\s*import\s+([^\s#]+)/gm,/^\s*from\s+([^\s]+)\s+import/gm]
    : language === 'java' ? [/^\s*import\s+([^;]+);/gm]
    : language === 'go' ? [/^\s*import\s+"([^"]+)"/gm]
    : [/\bimport\s+(?:[^'\"]+from\s+)?['\"]([^'\"]+)['\"]/g,/\brequire\(\s*['\"]([^'\"]+)['\"]\s*\)/g];
  const result: string[] = [];
  for (const p of patterns) { let m: RegExpExecArray|null; while ((m=p.exec(text))) if (m[1]) result.push(m[1]); }
...
export function indexRepository(repositoryId: string, revision: string, files: Array<{path:string; content:string}>): RepositoryIndex {
  ...
  for (const file of files) {
    ...
    const record: FileRecord = { path:file.path, language, size:file.content.length, hash:stableHash(file.content), imports, symbols:fileSymbols, tests: /(^|[/_.-])(test|tests|spec|specs)([/_.-]|$)/i.test(file.path) };
    records.push(record); symbols.push(...fileSymbols);
    for (const target of imports) dependencies.push({ from:file.path, to:target, kind: language === 'java' || language === 'python' ? 'import' : 'require' });
  }
  const counts = new Map<Language,number>();
  for (const f of records) if (f.language !== 'unknown') counts.set(f.language,(counts.get(f.language)??0)+1);
...
export class KnowledgeStore {
  ...
  search(query: RetrievalQuery): RetrievalResult[] {
    const terms = query.text.toLowerCase().split(/[^a-z0-9_$.-]+/).filter(t=>t.length>1);
    const results: RetrievalResult[] = [];
    for (const item of this.items.values()) {
      if (query.categories && !query.categories.includes(item.category)) continue;
      if (query.paths && !query.paths.some(p=>item.source.path?.startsWith(p))) continue;
      const hay = `${item.title} ${item.content} ${item.tags.join(' ')}`.toLowerCase();
      const matchedTerms = terms.filter(t=>hay.includes(t));
      if (!matchedTerms.length) continue;
      const score = matchedTerms.length / Math.max(1,terms.length) * 0.75 + item.importance * 0.25;
...
export function buildContextPack(store: KnowledgeStore, query: RetrievalQuery, budget: ContextBudget): ContextPack {
  const ranked = store.search({...query, limit: Math.max(query.limit??budget.maxItems,budget.maxItems*2)});
  const items: RetrievalResult[] = []; let chars=0;
  ...
  const provenance = [...new Map(items.map(r=>[r.item.source.sourceId, r.item.source.sourceId])).values()].map(sourceId=>({sourceId,categories:[...new Set(items.filter(r=>r.item.source.sourceId===sourceId).map(r=>r.item.category))]}));
