...
function validateRequest(request: ExecutionRequest): void {
  ...
  if (!request.command.length || request.command.some((part) => part.length === 0)) throw new Error('INVALID_COMMAND');
  const timeout = request.limits?.timeoutMs;
  if (timeout != null && (!Number.isInteger(timeout) || timeout < 1 || timeout > 300_000)) throw new Error('INVALID_TIMEOUT');
  const maxOutput = request.limits?.maxOutputBytes;
  if (maxOutput != null && (!Number.isInteger(maxOutput) || maxOutput < 1 || maxOutput > 16 * 1024 * 1024)) throw new Error('INVALID_OUTPUT_LIMIT');
...
function assertInside(root: string, target: string): string {
  ...
  if (rel === '' || (!isAbsolute(rel) && rel !== '..' && !rel.startsWith(`..${process.platform === 'win32' ? '\\' : '/'}`))) return targetAbs;
  throw new Error('WORKSPACE_PATH_ESCAPE');
}

function appendBounded(state: { text: string; bytes: number; truncated: boolean }, chunk: Buffer, max: number): void {
  if (state.bytes >= max) {
...
export class LocalExecutionProvider implements ExecutionProvider {
  ...
  async execute(request: ExecutionRequest): Promise<ExecutionResult> {
    ...
    const cwd = assertInside(this.policy.allowedRootForWorkspace(request.workspaceId), request.workingDirectory);
    ...
    const child = spawn(executable, request.command.slice(1), { cwd, env, shell: false, windowsHide: true });
    ...
    try {
      child.stdout.on('data', (chunk: Buffer) => appendBounded(stdout, chunk, limits.maxOutputBytes));
      child.stderr.on('data', (chunk: Buffer) => appendBounded(stderr, chunk, limits.maxOutputBytes));
...
export async function collectArtifact(root: string, path: string): Promise<ExecutionArtifact> {
  ...
  return { path: relative(resolve(root), safe), size: data.byteLength, sha256: createHash('sha256').update(data).digest('hex') };
}
