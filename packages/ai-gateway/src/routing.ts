...
export interface RouteDecision {
  ...
  rejected: readonly { provider: string; model: string; reason: string }[];
...
export class ModelRegistry {
  ...
  register(model: ModelMetadata): void {
    if (!/^[a-z0-9._:-]{1,160}$/.test(model.id)) throw new Error('INVALID_MODEL_ID');
    if (!/^[a-z0-9_-]{1,64}$/.test(model.provider)) throw new Error('INVALID_PROVIDER_ID');
    if (model.contextWindow < 1 || model.maxOutputTokens < 1) throw new Error('INVALID_MODEL_LIMITS');
    if (model.pricing && (model.pricing.inputPerMillion < 0 || model.pricing.outputPerMillion < 0))
  ...
  replace(model: ModelMetadata): void {
    if (!this.models.has(model.id)) throw new Error(`MODEL_NOT_FOUND:${model.id}`);
    this.models.set(model.id, { ...model, capabilities: new Set(model.capabilities) });
  }
  ...
  list(): ModelMetadata[] {
    return [...this.models.values()]
      ...
      .map((model) => ({ ...model, capabilities: new Set(model.capabilities) }));
  }
}
...
export class HealthTracker {
  ...
  constructor(private readonly failureThreshold = 2, private readonly cooldownMs = 30_000) {
    if (failureThreshold < 1 || cooldownMs < 0) throw new Error('INVALID_HEALTH_POLICY');
  }

  get(provider: string): ProviderHealthState {
    return this.states.get(provider) ?? { provider, available: true, consecutiveFailures: 0 };
  }

  recordSuccess(provider: string): void {
    this.states.set(provider, { provider, available: true, consecutiveFailures: 0 });
  }
...
export class Router {
  ...
  decide(
    ...
    for (const model of this.registry.list()) {
      if (!model.enabled) {
        rejected.push({ provider: model.provider, model: model.id, reason: 'MODEL_DISABLED' });
        continue;
      }
      if ([...required].some((capability) => !model.capabilities.has(capability))) {
        rejected.push({ provider: model.provider, model: model.id, reason: 'CAPABILITY_MISMATCH' });
        continue;
      }
      if (excluded.has(model.provider)) {
        rejected.push({ provider: model.provider, model: model.id, reason: 'PROVIDER_EXCLUDED' });
        continue;
      }
      if (!this.health.isAvailable(model.provider, now.getTime())) {
        rejected.push({ provider: model.provider, model: model.id, reason: 'PROVIDER_UNHEALTHY' });
        continue;
      }
      const matchingCredentials = activeCredentials.filter((credential) => credential.provider === model.provider);
      if (!matchingCredentials.length) {
        rejected.push({ provider: model.provider, model: model.id, reason: 'NO_ELIGIBLE_CREDENTIAL' });
        continue;
      }
      if (policy.maxLatencyMs != null && (model.expectedLatencyMs ?? Number.POSITIVE_INFINITY) > policy.maxLatencyMs) {
        rejected.push({ provider: model.provider, model: model.id, reason: 'LATENCY_LIMIT' });
        continue;
      }
      for (const credential of matchingCredentials) {
        const quota = credential.quota?.remainingFraction ?? 1;
        if (quota <= 0) {
          rejected.push({ provider: model.provider, model: model.id, reason: 'QUOTA_EXHAUSTED' });
          continue;
        }
        const cost = this.costScore(model);
        if (policy.maxEstimatedCost != null && model.pricing && model.pricing.inputPerMillion > policy.maxEstimatedCost) {
          rejected.push({ provider: model.provider, model: model.id, reason: 'COST_LIMIT' });
          continue;
        }
        const factors = {
          cost,
        ...
        const totalWeight = weights.cost + weights.latency + weights.reliability + weights.preference;
        const score = (factors.cost * weights.cost + factors.latency * weights.latency + factors.reliability * weights.reliability + factors.preference * weights.preference) / totalWeight;
...
export interface RoutedCompletion {
  ...
  attempts: readonly { provider: string; credentialId: string; model: string; error?: string }[];
...
export class RoutedAIGateway {
  ...
  async complete(
    ...
    const attempts: Array<{ provider: string; credentialId: string; model: string; error?: string }> = [];
    ...
    for (const candidate of chain) {
      ...
      try {
        ...
        attempts.push({ provider: candidate.model.provider, credentialId: candidate.credential.credentialId, model: candidate.model.id });
        return {
          response,
          decision,
          attempts,
          estimatedCost: estimateCost(candidate.model, response.usage),
        };
      } catch (error) {
        attempts.push({
...
function clonePolicy(policy: RoutingPolicy): RoutingPolicy {
  return {
    ...policy,
    requiredCapabilities: policy.requiredCapabilities ? new Set(policy.requiredCapabilities) : undefined,
    preferredProviders: policy.preferredProviders ? [...policy.preferredProviders] : undefined,
    excludedProviders: policy.excludedProviders ? [...policy.excludedProviders] : undefined,
    weights: policy.weights ? { ...policy.weights } : undefined,
  };
}
...
function isRetryableProviderError(error: unknown): boolean {
  return Boolean(error && typeof error === 'object' && 'retryable' in error && (error as AIProviderError).retryable);
}
