...
function classify(job: CiJobResult): { kind: FailureKind; severity: FailureSeverity; summary: string } {
  ...
  if (job.status === 'cancelled' || /timeout|timed out|deadline exceeded/.test(text)) {
    return { kind: 'timeout', severity: 'high', summary: 'CI execution timed out or was cancelled.' };
  }
  ...
  if (/dependency|lockfile|pnpm install|npm install|resolution/.test(text)) {
    return { kind: 'dependency', severity: 'high', summary: 'A dependency or lockfile step failed.' };
  }
  ...
  if (/tsc|typecheck|type error|cannot find module|syntaxerror|compile/.test(text)) {
    return { kind: 'compile', severity: 'high', summary: 'Compilation or type verification failed.' };
  }
  ...
  if (/runner|network|enoent|econn|permission denied|out of memory/.test(text)) {
    return { kind: 'environment', severity: 'high', summary: 'The CI environment or infrastructure failed.' };
  }
  return { kind: 'unknown', severity: 'medium', summary: 'CI failed without a recognized failure signature.' };
}
...
export function validateCiRun(run: CiRunRef): string[] {
  ...
  if (!run.repository.name || !idPattern.test(run.repository.name)) errors.push('INVALID_REPOSITORY');
...
export async function analyzeCiRun(run: CiRunRef, input: CiAnalysisInput): Promise<CiAnalysis> {
  ...
  const jobs = (await input.adapter.getJobs(currentRun)).slice().sort((a, b) => a.jobId.localeCompare(b.jobId));
