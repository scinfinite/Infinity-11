...
export class MemoryStore {
  ...
  put(record: MemoryRecord): void {
    this.records.set(record.id, { ...record, tags: [...new Set(record.tags)] });
  }
  ...
  search(query: MemoryQuery): MemoryRecord[] {
    ...
    return this.list(query.projectId)
      ...
      .map((r) => ({
        r,
        score:
          terms.reduce((n, t) => n + (r.content.toLowerCase().includes(t) ? 1 : 0), 0) *
          r.confidence *
          r.importance,
      }))
      .filter((x) => x.score > 0)
...
export function memoryFromLesson(
  projectId: string,
  content: string,
  sourceRef: string,
  importance = 0.8,
  ...
  return {
    id: `lesson:${projectId}:${sourceRef}`,
    projectId,
    type: 'lesson',
    content,
    tags: ['engineering'],
    source: { kind: 'verification', ref: sourceRef },
    confidence: 1,
    importance,
    createdAt: now,
    updatedAt: now,
