...
describe('Stage 8 Project Brain', () => {
  ...
  it('indexes files, symbols, imports and tests', () => {
    ...
    expect(brain.index.files.find((file) => file.path.endsWith('.test.ts'))?.tests).toBe(true);
  });
  ...
  it('ranks knowledge and enforces context budgets with provenance', () => {
    ...
    store.upsert({
      ...
      content: 'Use durable event storage',
      source: { sourceId: 'architecture', path: 'docs/architecture.md' },
      tags: ['database'],
      importance: 1,
    ...
    store.upsert({
      ...
      category: 'lesson',
      title: 'Failed retry',
      content: 'Never replay destructive side effects',
      source: { sourceId: 'verification', path: 'tests.md' },
      tags: ['recovery'],
      importance: 0.9,
      updatedAt: new Date().toISOString(),
    });
