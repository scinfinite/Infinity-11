...
const model = (overrides: Partial<ModelMetadata> = {}): ModelMetadata => ({
  ...
  reliability: 0.99,
  provenance: { source: 'test-fixture', observedAt: '2026-09-14T00:00:00.000Z', confidence: 'high' },
  enabled: true,
  ...overrides,
});
...
describe('stage 4 model routing', () => {
  it('filters capabilities, disabled models, unhealthy providers, and exhausted quotas', () => {
    ...
    const decision = router.decide('req-1', [credential({ quota: { remainingFraction: 1 } })], {
  ...
  it('is deterministic for identical registry, credentials, and policy inputs', () => {
    ...
    const router = new Router(registry);
    const candidates = [credential(), credential({ credentialId: 'credential-b', provider: 'provider-b' })];
    const policy = { preferredProviders: ['provider-b'] };
    const first = router.decide('req', candidates, policy, new Date('2026-09-14T00:00:00.000Z'));
    const second = router.decide('req', candidates, policy, new Date('2026-09-14T00:00:00.000Z'));
  ...
  it('keeps fallback attempts bounded and retries only retryable failures', async () => {
    const registry = new ModelRegistry();
    registry.register(model({ provider: 'provider-a' }));
    registry.register(model({ id: 'model-b', provider: 'provider-b' }));
    const router = new Router(registry);
    const sink = new InMemoryRouteDecisionSink();
    const complete = vi
      .fn()
      .mockRejectedValueOnce({ retryable: true, message: 'temporary' })
      .mockResolvedValueOnce({ id: 'response-1', provider: 'provider-b', model: 'model-b', text: 'ok' });
    const gateway = { complete } as unknown as AIGateway;
    const routed = new RoutedAIGateway(gateway, router, sink);

    const result = await routed.complete(
      { workspaceId: 'workspace', messages: [{ role: 'user', content: 'hello' }] } as Omit<AIRequest, 'credentialId' | 'model'>,
  ...
  it('does not fall back on non-retryable provider failures', async () => {
    ...
    registry.register(model({ id: 'model-b', provider: 'provider-b' }));
    const router = new Router(registry);
    const complete = vi.fn().mockRejectedValue({ retryable: false, message: 'bad request' });
